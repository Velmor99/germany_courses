...
const SortOfConsultation = ({textOptions}: SortOfConsultationsProps) => {
  ...
  const generateData = () => {
    ...
    const genereted = keys.map(
      (item) => `sort_of_consultation_items.${item}`
    );
    return genereted;
  };
  return (
      ...
      <div className={cn(styles["container"])}>
        <div className={cn(styles["sort-of__content-block"])}>
          <p className={cn(styles["sort-of__text"])}>
            <TextComponent text={t("sort_of_consultation_text")} targets={textOptions.targets} textColor={textOptions.colorText} fontWeight={textOptions.fontWeight} />
          </p>
          <div className={cn(styles["sort-of__image"])}>
            <div className={cn(styles["sort-of__we-icon"])}></div>
          </div>
        </div>
        <h2 className={cn(styles["sort-of__title"])}>{t("sort_of_consultation_title")}</h2>
