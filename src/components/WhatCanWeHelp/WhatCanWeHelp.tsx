  ...
  return (
    <div className={cn(styles["help"])}>
      <div className={cn(styles["container"])}>
        <h2 className={cn(styles["help__title"])}>{t("what_can_we_help_title")}</h2>
        <ul className={cn(styles["help__list"])}>
          {generateData().map((item, idx) => 
          <li key={idx} className={cn(styles[`help__list-item`], styles[`help__list-item-${idx + 1}`])}>
            <WhatCanWeHelpCard className={cn(styles[`card_${idx + 1}`])} text={t(item)} />
          </li>
          )}
        </ul>
      </div>
