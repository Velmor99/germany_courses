...
const CountrySelect = ({ className, callback }: CountrySelectProps): JSX.Element => {
  ...
  const changeFlag = (country: string) => {
    setCurrentFlag(`http://purecatamphetamine.github.io/country-flag-icons/3x2/${country}.svg`);
    callback(country)
  ...
  return (
    <div className={cn(styles["select"], className)}>
      <div
      onClick={() => changeFlagMenu(!isCountriesMenuOpen)}
      >
        <Image className={cn(styles["select__flag-icon"])} alt="country flag" width={38} height={27} src={currentFlag} />
      </div>
      {isCountriesMenuOpen && (
